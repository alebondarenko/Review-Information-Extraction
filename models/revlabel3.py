...
def makemodel(is_training):
	...
	lstm_output_tup, final_state_tup = tf.nn.bidirectional_dynamic_rnn(cell_fw=cell, cell_bw=cell, sequence_length=np.full(flags.batchsize, flags.numsteps), inputs=e, initial_state_fw=initial_state_fw, initial_state_bw=initial_state_bw)
	lstm_output = tf.concat([lstm_output_tup[0], lstm_output_tup[1]], axis=2)
	outputs = tf.transpose(lstm_output, [1, 0, 2])

	model_outputs = list()
	W = tf.get_variable('weights', shape=[2*lstmdim, flags.aembdim], dtype=tf.float32)
	...
	modeldict = {'X': X, 'e_w': e_w, 'e_p': e_p, 'target': target, 'initial_state_fw': initial_state_fw, 'initial_state_bw': initial_state_bw, 'final_state_fw': final_state_tup[0], 'final_state_bw': final_state_tup[1], 'loss': loss, 'optimizer': optimizer, 'lr': lr, 'outputs': model_outputs}
	return modeldict
...
def myfileprint(s):
	if not os.path.exists('../results/model3/data1/'): os.makedirs('../results/model3/data1/')
	f = codecs.open('../results/model3/data1/training-log.txt', 'a', 'utf-8')
	f.write(s + '\n')
	f.close()
...
def runepoch(sess, data, modeldict, fetches, epoch_no, verbose, record):
	...
	state_fw = sess.run(modeldict['initial_state_fw'])
	state_bw = sess.run(modeldict['initial_state_bw'])
	losses = 0.0
	...
	for curr, x in enumerate(data_iterator(data, flags.batchsize)):
		feed_dict = {modeldict['X']: x, modeldict['initial_state_fw']: state_fw, modeldict['initial_state_bw']: state_bw}
		...
		if record: 
			...
			f = codecs.open('../results/model3/data1/record-' + str(flags.step)  + '.txt', 'a', 'utf-8')
			newlist = newx.tolist()
			for sent in newlist:
...
def train(traindata):
	...
	with tf.Session() as sess:
		...
		fetches = dict()
		fetches['final_state_fw'] = m_train['final_state_fw']
		fetches['final_state_bw'] = m_train['final_state_bw']
		...
		tf.train.write_graph(sess.graph_def, '../modelparams/model3/data1/', 'model.pb', as_text=False)
		sess.run(tf.global_variables_initializer())
		# saver.restore(sess, '../modelparams/model3/data1/checkpoint-10') # Un-comment this, to start from a custom pre-trained model state
		for i in range(flags.max_max_epoch):
			runepoch(sess, traindata, m_train, fetches, i, True, False)
			save_path = saver.save(sess, '../modelparams/model3/data1/model')
			save_path = saver.save(sess, '../modelparams/model3/data1/checkpoint-' + str(i)) # make sure, that when loading a custom model, this is adjusted correctly
...
def test(testdata):
	with tf.Session() as sess:
		...
		fetches = dict()
		fetches['final_state_fw'] = m_test['final_state_fw']
		fetches['final_state_bw'] = m_test['final_state_bw']
		fetches['loss'] = m_test['loss']		
		fetches['outputs'] = m_test['outputs']
		saver = tf.train.Saver(max_to_keep=200)
		saver.restore(sess, '../modelparams/model3/data1/checkpoint-' + str(int(flags.step) - 1))
		# saver.restore(sess, '../modelparams/model3/data1/model')
